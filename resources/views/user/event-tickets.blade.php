...
<style>
    ...
    .qty-control { width: 140px; display: flex; align-items: center; justify-content: space-between; background: rgba(0,0,0,0.3); border-radius: 50px; padding: 5px; border: 1px solid rgba(255,255,255,0.1); }
    ...
    .order-item-name { color: #cbd5e1; flex: 1; padding-right: 8px; }
    .order-item-price { color: #F4D03F; font-weight: 700; white-space: nowrap; }
    .order-total-row {
        display: flex;
        justify-content: space-between;
        align-items: center;
    ...
    .order-total-label { color: #a0aec0; font-size: 0.8rem; font-weight: 700; text-transform: uppercase; }
    .order-total-amount { color: #F4D03F; font-size: 1.5rem; font-weight: 900; }
    ...
    .btn-checkout-sidebar:hover { transform: translateY(-2px); box-shadow: 0 8px 25px rgba(244, 208, 63, 0.5); color: #000; }
    .order-empty-state { text-align: center; padding: 20px 0; color: #a0aec0; font-size: 0.88rem; }
    .order-empty-state i { font-size: 2rem; margin-bottom: 10px; opacity: 0.3; display: block; }
...
<div class="container">
    ...
    <form action="{{ route('cart.add') }}" method="POST" id="bulkTicketForm">
        ...
        <div class="container mt-4">
            ...
            {{-- Two Column: Ticket List + Order Summary --}}
            <div class="row g-4">
                                            ...
                                            @if($ticket->capacity > 0)
                                                ...
                                                <div class="qty-control">
                                                    ...
                                                    <input type="number" name="tickets[{{ $ticket->id }}]" 
                                                           ...
                                                           data-ticket-name="{{ $ticket->ticket_type->name }}"
                ...
                <div class="col-lg-4">
                    <div class="order-summary-card">
                        <div class="order-summary-title">
                            <i class="fas fa-shopping-bag me-2" style="color: #F4D03F;"></i> Ringkasan Pesanan
                        </div>
                        ...
                        {{-- Divider + Total --}}
                        <div id="orderTotalSection" style="display:none;">
                            <div class="order-total-row">
                                <div>
                                    <div class="order-total-label">Total</div>
                                    <div class="order-total-amount">Rp <span id="summaryTotalAmount">0</span></div>
                                </div>
                                <div class="text-end">
                                    <div class="order-total-label">Jumlah</div>
                                    <div style="color:#fff; font-weight:800; font-size:1.2rem;"><span id="summaryTicketCount">0</span> Tiket</div>
                                </div>
                            </div>
                            <button type="submit" class="btn-checkout-sidebar" id="checkoutSidebarBtn" style="display:block;">
                        ...
                        <div class="mt-4 pt-3" style="border-top: 1px solid rgba(255,255,255,0.06);">
                            <div style="font-size:0.78rem; color:#a0aec0; text-transform:uppercase; font-weight:700; letter-spacing:1px; margin-bottom:8px;">Event</div>
                            <div style="font-weight:700; color:#fff; font-size:0.9rem;">{{ $event->name }}</div>
                            <div style="font-size:0.82rem; color:#a0aec0; margin-top:4px;"><i class="fas fa-map-marker-alt me-1"></i>{{ $event->location }}</div>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    function updateOrderSummary() {
        ...
        allRows.forEach(row => {
            ...
            if (qty <= 0) return;
            const price = parseInt(qtyInput.getAttribute('data-ticket-price') || row.getAttribute('data-price')) || 0;
            const name  = qtyInput.getAttribute('data-ticket-name') || 'Tiket';
            totalCount  += qty;
            totalAmount += qty * price;
            itemsHtml   += `
                <div class="order-item-row">
                    <div class="order-item-name">${name} <span style="color:#a0aec0;">×${qty}</span></div>
                    <div class="order-item-price">Rp ${new Intl.NumberFormat('id-ID').format(qty * price)}</div>
                </div>`;
        });
        ...
        if (totalCount > 0) {
            ...
            summaryTotal.innerText     = new Intl.NumberFormat('id-ID').format(totalAmount);
